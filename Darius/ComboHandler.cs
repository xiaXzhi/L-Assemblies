...
namespace Darius
{
    internal class ComboHandler
    {
        ...
        {
            foreach (var ksTarget in ObjectManager.Get<Obj_AI_Hero>().Where(ksTarget => R.CanCast(ksTarget)))
                CastR(ksTarget);
        }
        ...
        {
            if (!SpellHandler.IsEnabled("comboUseW") || !ConfigHandler.KeyLinks["comboActive"].Value.Active || !unit.IsMe || !(target is Obj_AI_Hero))
                return;
